...
function communityId(type: CommunityType, referenceId: string) {
  return `community-${type}-${normalize(referenceId)}`;
}
...
function ensureCommunity(
  ...
  const id = communityId(type, referenceId);
  ...
  buildCommunities(students: Student[], courses: Course[]): AssignmentResult {
    ...
    students.forEach((student) => {
      ...
      const className = student.program
        ...
        : student.class_group;
      const classCommunity = ensureCommunity(communities, "class", className, student.class_group);
      ...
      unique(student.enrolledCourseIds).forEach((courseId) => {
        ...
        const courseCommunity = ensureCommunity(communities, "course", course.name, referenceId);
